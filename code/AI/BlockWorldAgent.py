    ...
    def solve(self, initial_arrangement, goal_arrangement):
        ...
        class State:
            ...
            def select_move(self):  
                ...
                for index, stack in enumerate(self.second_stack):
                    for index2, stack2 in enumerate(self.first_stack):
                        if stack:  
                            curr_table, new_second, move = self.valid_state_move2(self.second_stack, index, self.first_stack, index2)
                            new_state = State(curr_table, new_second, self.total_num, copy.copy(self.moves))
                            new_state.moves.append(move)
                            if new_state.difference() < self.difference():
                                return new_state
            ...
            def valid_state_move2(self, table, start_index, table2, end_index):
                temp_table = copy.deepcopy(table)
                temp_table2 = copy.deepcopy(table2)
                left = temp_table[start_index]
                top_block = left.pop()
                right = temp_table2[end_index]
                ...
                if len(left) == 0:
                    temp_table.remove(left)
                return temp_table2, temp_table, move
