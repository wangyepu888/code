...
training_images = np.loadtxt("58Train_Images.gz") #x
labels = np.loadtxt("58Train_Labels.gz") #y
...
def train_example(imags, labels):
    global w
    y=labels
    y=y.reshape(len(y),1) 
    for j in range(785):
       for itineration in range(1000):
           i=np.random.randint(low=0,high=numImages,size=1)
           f=imags[i].reshape(785,1)
           m=sigmoid(np.dot(w.T,f))-y[i]
           w[j]=w[j]-L*imags[i,j]*m
        
  

train_example(training_images,labels)
print(w)
